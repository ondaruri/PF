class HomesController < ApplicationController
  def top
    ...
    @tonkothu_ranks = PostRamen.joins(:likes).tagged_with("とんこつ").where(likes: { created_at: now.beginning_of_week..now.end_of_week}).group(:id).order("count(post_ramen_id) desc").limit(10) #とんこつランキング
    @shio_ranks = PostRamen.joins(:likes).tagged_with("しお").where(likes: { created_at: now.beginning_of_week..now.end_of_week}).group(:id).order("count(post_ramen_id) desc").limit(10) #しおランキング
  end
end
